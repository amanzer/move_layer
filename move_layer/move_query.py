...
class MoveQuery:
    ...
    def create_temporal_view(self, project_title, db, col_id):
        if self.column_types[col_id] == 'tgeometry':
            ...
            v, s = self.get_database_parameters(col_id)
            return 0, v, s  # TODO 
    ...
    def get_database_parameters(self, col_id): # TODO
        """
        For Tgeompoints : retrieve the table, column names
        """
        sql_parts = []
        if self.has_with:
            sql_parts.append(self.with_sql)
